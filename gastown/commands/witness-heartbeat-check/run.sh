#!/bin/sh
# gastown witness-heartbeat-check — is each witness's patrol loop still beating?
# Invoked as: gc gastown witness-heartbeat-check
#
# This wrapper exists for one reason: path resolution. The check itself lives in
# assets/scripts/witness-heartbeat-check.sh, but its caller is the deacon's
...
# which makes `gc gastown witness-heartbeat-check` the only invocation that can
...
# check into a permanent no-op that reads like a considered fallback. That is
# the defect usage-stamp was routed through `gc` to fix (gp-fid); the deacon's
# heartbeat check had the same wiring and never ran once (gp-3qb).
#
# The staleness window stays the caller's: GASTOWN_WITNESS_STALE_MIN and
# GASTOWN_WITNESS_ROLE pass through the environment, exactly as they did when
...
#   0 = every checked witness heartbeat is inside the window (or none to check)
...
if [ -z "${GC_CITY_PATH:-}" ] || [ -z "${GC_PACK_DIR:-}" ]; then
    ...
    echo "  Run this as 'gc gastown witness-heartbeat-check' — invoking run.sh" >&2
...
if [ ! -r "$CHECK" ]; then
    echo "gc gastown witness-heartbeat-check: $CHECK not found in this pack version" >&2
    exit 2
fi
