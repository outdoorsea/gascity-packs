...
slack is a Slack provider extension for Gas City. When this directory is
mirrored to `gastownhall/gascity-packs/slack/`, the root repo's contributing
guide is the source of truth — start there, then read the slack-specific
notes below.
...
CI runs all three on every PR that touches `examples/slack/**` (see
`.github/workflows/slack.yml`).

## Secret handling

slack reads Slack credentials from environment variables only. Never
...
- Keep PRs scoped to slack (or paired adapter changes when needed).
