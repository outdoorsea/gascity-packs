...
This pack lives at `slack/` in the
...
- **Adapter** — `slack/adapter/gc-slack-adapter`,
...
- **Operator CLI** — `slack/cli/gc-slack-cli`, built
...
The Go adapter at `slack/adapter/gc-slack-adapter`
...
# city.toml
[imports.slack]
source = "/path/to/gascity-packs/slack"
```
...
`slack/adapter/main.go` reads — is enumerated in the
...
( cd slack/adapter && go build -o gc-slack-adapter )
...
# 4. Reload the city so the [[service]] block from slack registers
