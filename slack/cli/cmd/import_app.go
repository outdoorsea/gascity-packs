...
func NewImportAppCmd(stdout, _ io.Writer) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short: "Import a Slack app manifest into the gc city's slack registry",
		Long: `Import a Slack app manifest into the gc city's slack registry.

Reads the JSON manifest at <manifest.json>, validates that it declares
the bot scopes the slack adapter and downstream commands require,
