...
// <cityPath>/.gc/slack/room_launch_mappings.json. The slack adapter
...
Gas City has ZERO hardcoded role names; the slack adapter passes
...
field, so it must match an agent template the city actually configures.`,
	...
	cmd.Flags().StringVar(&poolTemplate, "launcher", "",
		"Pool template (agent name) the slack adapter spawns for `@@<handle>` posts in this channel (required)")
	if defaultWorkspace == "" {
...
// and map-rig verbs because the slack adapter loads every registry
// once at startup.
