...
`examples/slack/cli/cmd/<cmd>.go` and registers it on the
cobra root in `examples/slack/cli/main.go`.
...
| ----------------------------------------- | ---------------------------------------------------- |
| `cmd/gc/cmd_slack_<cmd>.go`               | `examples/slack/cli/cmd/<cmd>.go`               |
| `cmd/gc/cmd_slack_<cmd>_test.go`          | `examples/slack/cli/cmd/<cmd>_test.go`          |
| `cmd/gc/main.go` cobra registration       | `examples/slack/cli/main.go` cobra registration |
...
coupling so the slack remains a self-contained example. If a
...
Keep the helper in `examples/slack/cli/cmd/citypath.go` and
...
- **`cmd/gc/main.go`** subcommand registration — keep the gc CLI
  ...
  verb is ported.
- **`examples/slack/commands/*.sh`** wrappers — the pack still
...
- **`pack.toml`** — same reasoning. Pack-level wiring changes are
  Phase 2 work.
- **`examples/slack/cli/internal/state/`** — the helpers landed
...
- `examples/slack/cli/cmd/<cmd>.go` compiles
  (`cd examples/slack/cli && go build ./...` is green)
- `examples/slack/cli/cmd/<cmd>_test.go` passes
  (`go test -race ./cmd/...` is green)
- `cd examples/slack/cli && go vet ./...` clean
- `gofmt -l examples/slack/cli/cmd/` reports no diffs
- `examples/slack/cli/main.go` registers the new subcommand under
...
Each Phase 1 leaf appends ONE line to `examples/slack/cli/main.go`
inside `newRootCmd()`:
...
- The leaf that lands the very first verb also creates
  `examples/slack/cli/cmd/` and adds the `cmdpkg` import to
...
- Deleting `cmd/gc/cmd_slack_*.go` and `cmd/gc/slack_*.go` originals
  — Phase 2.
- Updating `examples/slack/commands/*.sh` wrappers — Phase 2.
