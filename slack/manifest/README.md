...
Canonical Slack app manifest for the slack. `app.json` is the
**source of truth** for the Slack app's display info, OAuth scopes,
event subscriptions, and (eventually) slash commands. Use it to
install the slack into a fresh workspace without clicking through
...
- **`features.slash_commands`** — empty list. Slash commands will be
  ...
  that command lands. Until then, the slack does not expose any
  `/gc …` shortcuts in Slack.
- **`oauth_config.scopes.bot`** — the minimal scope set the live
  adapter (`examples/slack/adapter/main.go`) requires today.
...
```bash
gc slack import-app examples/slack/manifest/app.json \
  --workspace-id T0123456 \
  --app-id       A0123456
```

This validates the manifest's bot scopes against the set the slack
...
the contract between the gc CLI (writer) and the slack adapter
(reader).
...
```bash
python3 -c "import json; json.load(open('examples/slack/manifest/app.json'))"
```

The pytest suite in `examples/slack/tests/test_manifest.py`
asserts the file parses + carries the required top-level keys; run it
with `pytest examples/slack/tests/test_manifest.py`.
