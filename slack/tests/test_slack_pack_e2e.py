"""End-to-end pipeline tests for slack.

Wires ``GcMock`` and ``SlackMock`` together to exercise the full
``slack script -> gc /extmsg/* -> http_adapter -> Slack`` round-trip
...
Coverage rationale (gastownhall/gascity#1817 + #1802 context):
  ...
  * #1802 (binding fallback): the slack script's binding lookup
...
def test_reply_current_publishes_via_group_membership_fallback(
    ...
    """Pin the gastownhall/gascity#1802 group-fallback contract from the slack side.
    ...
    The test exercises this from slack's vantage point:
...
def test_outbound_returns_auth_failure_when_session_lacks_binding_and_group(
    ...
    This pins the response shape slack consumers must handle. Surfaces
...
def test_reply_current_resolves_conversation_from_inbound_event(
    ...
    Pipeline:
      1. A synthetic extmsg.inbound event is seeded in GcMock targeting our
         session (modeling: a Slack message arrived, the slack adapter
