"""In-process Slack Web API mock for slack E2E tests.

Captures every ``/api/chat.*`` POST so tests can assert on what the
slack adapter actually sent across the wire, and synthesizes
...
slack pipeline. It is not a fidelity reimplementation of the
Slack Web API.
...
gastownhall/gascity#1817 — the slack adapter callback URL points
...
class SlackMock:
    ...
    @property
    def url(self) -> str:
        """Base URL of the mock — point the slack adapter's outbound HTTP client here."""
