// OAuth install flow for slack (gc-cby.9).
//
...
// rejects the request.
//
// In-process: slack runs a single adapter, so this is sufficient.
...
func handleOAuthCallback(cfg oauthConfig, reg *appsRegistry) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w,
			"slack installed for workspace %s (%s), app %s.\n\n"+
...
func writeInstallEnv(cityPath, workspaceID, appID, botToken string) (string, error) {
	...
	body := fmt.Sprintf(
		"# Written by slack OAuth install flow (gc-cby.9).\n"+
