...
```bash
cd <gascity-repo>/examples/slack/adapter
./run.sh
```
...
ExecStart=<gascity-repo>/examples/slack/adapter/run.sh
Restart=on-failure
RestartSec=5
...
`proxy_process`), follow the cutover sequence in the slack README
